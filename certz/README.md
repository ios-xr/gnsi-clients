...
A complete Go client implementation for the gNSI (gRPC Network Security Interface) Certificate Management Service (Certz). Designed for Cisco IOS XR and other gNSI-compliant network devices.
...
| `-client_cert` | string | - | Client certificate file path (optional, enables certificate-based authentication).<br/>**Supports certificate chains**: file can contain leaf + intermediate certificates in PEM format |
| `-client_key` | string | - | Client private key file path (optional, required with `-client_cert`).<br/>Must be the private key for the leaf certificate |
...
| Flag | Type | Default | Description |
|------|------|---------|-------------|
| `-op` | string | get-profile-list | Operation to perform. Options:<br/>• `add-profile` - Create new SSL profile<br/>• `delete-profile` - Remove SSL profile<br/>• `get-profile-list` - List all SSL profiles<br/>• `can-generate-csr` - Check if device can generate CSR<br/>• `rotate` - Rotate certificate/CA/CRL |
...
| `-cert_file` | string | - | Certificate file path (PEM format) for rotation.<br/>**Supports certificate chains**: file can contain leaf + intermediate certificates |
| `-key_file` | string | - | Private key file path (PEM format) for rotation.<br/>Must be the private key for the leaf certificate |
| `-cert_source` | string | - | Use device-native certificate. Options:<br/>• `IDevID` - Initial Device Identifier (manufacturer cert)<br/>• `oIDevID` - Operational IDevID<br/>• `self-signed` - Device-generated certificate |
...
| `-csr_suite` | string | rsa-2048-sha256 | CSR suite (key type and signature algorithm). Options:<br/>**RSA 2048:**<br/>• `rsa-2048-sha256`<br/>• `rsa-2048-sha384`<br/>• `rsa-2048-sha512`<br/>**RSA 3072:**<br/>• `rsa-3072-sha256`<br/>• `rsa-3072-sha384`<br/>• `rsa-3072-sha512`<br/>**RSA 4096:**<br/>• `rsa-4096-sha256`<br/>• `rsa-4096-sha384`<br/>• `rsa-4096-sha512`<br/>**ECDSA P-256:**<br/>• `ecdsa-p256-sha256`<br/>• `ecdsa-p256-sha384`<br/>• `ecdsa-p256-sha512`<br/>**ECDSA P-384:**<br/>• `ecdsa-p384-sha256`<br/>• `ecdsa-p384-sha384`<br/>• `ecdsa-p384-sha512`<br/>**ECDSA P-521:**<br/>• `ecdsa-p521-sha256`<br/>• `ecdsa-p521-sha384`<br/>• `ecdsa-p521-sha512`<br/>**EdDSA:**<br/>• `eddsa-ed25519` |
...
| `-entity_type` | string | - | Type of entity to copy. Options:<br/>• `cert` - Copy certificate chain<br/>• `ca_bundle` - Copy CA trust bundle<br/>• `crl_bundle` - Copy CRL bundle |
