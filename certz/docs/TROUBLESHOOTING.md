...
3. **Verify PEM format:**
   ...
   **Note:** Certificate files support chains (multiple certificates). If using a chain, the file should contain:
   ```pem
   -----BEGIN CERTIFICATE-----
   ...
   -----END CERTIFICATE-----
   ```
   The private key file must contain ONLY the private key for the leaf certificate.
